...
# -------------------------------------------   Data   -----------------------------------------------------------------
train_Data = my_Data_Set(r'./labels/split_labels_91/train_split.txt',
...
val_Data = my_Data_Set(r'./labels/split_labels_91/val_split.txt',
                       transform=data_transforms['val'],
                       loader=Load_Image_Information_Val)
...
test_freq = 500
save_freq = 5  # save model at every 10 epochs
...
while True:
    ...
    for imgs, targets in train_DataLoader:
        ...
        if iteration % test_freq == 0:
            ...
            print("epoch:{:2d} iter:{:3d} val: "
                  "micro precision: {:.3f} "
                  "macro precision: {:.3f} "
                  "samples precision: {:.3f}".format(epoch, iteration,
            ...
            print("epoch:{:2d} iter:{:3d} val: "
                  "micro recall: {:.3f} "
                  "macro recall: {:.3f} "
                  "samples recall: {:.3f}".format(epoch, iteration,
                                                  result['micro/recall'],
                                                  result['macro/recall'],
    ...
    if (epoch + 1) % save_freq == 0:
        ...
        torch.save(model.state_dict(), 'The_' + str(epoch) + '_epoch_ResNext' + date + "_" + n_experiments + '.pkl')
        print("Save model:" + 'The_' + str(epoch) + '_epoch_ResNext' + date + "_" + n_experiments + '.pkl')
    epoch += 1
