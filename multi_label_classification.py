# -*- coding: utf-8 -*-
"""pytorch-resnet50-starter.ipynb
...
print("os.listdir(train_dir)[number].split('.')[0]",os.listdir(train_dir)[number].split('.')[0])
print("labels_dict[os.listdir(train_dir)[number].split('.')[0]]",labels_dict[os.listdir(train_dir)[number].split('.')[0]])
...
class ImageData(data.Dataset):
    def __init__(self,df,dirpath,transform,test = False):
        ...
        #image data 
        if not self.test:
            self.image_arr = np.asarray(str(self.dirpath)+'/'+self.df.iloc[:, 0]+'.jpg')
