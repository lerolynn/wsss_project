...
def cam_to_mask(img, mask,name):
	heatmap = cv2.applyColorMap(np.uint8(255*mask), cv2.COLORMAP_JET)
	heatmap = np.float32(heatmap) / 255
	cam = heatmap + np.float32(img)
	cam = mask
	cv2.imwrite("mask/cam_{}.jpg".format(name), np.uint8(255 * cam))
...
if __name__ == '__main__':
	...
	for s in filename:
		...
		image_dict[str(s.split(".")[0])] = cv2.imread(args.image_path + s,1)
	...
	for key, img in image_dict.items():
		...
		i=i+1 
		show_cam_on_image(img, mask, key)

		#  Load the trained model
		pretrained_weights = torch.load('../models/resnet50_food.pth')
		...
		if not os.path.exists('camgb'):
			os.mkdir('camgb')
		utils.save_image(torch.from_numpy(gb), 'gb/gb_{}.jpg'.format(key))
		cam_mask = np.zeros(gb.shape)
		for j in range(0, gb.shape[0]):
			cam_mask[j, :, :] = mask
		cam_gb = np.multiply(cam_mask, gb)
		utils.save_image(torch.from_numpy(cam_gb), 'camgb/cam_gb_{}.jpg'.format(key))
