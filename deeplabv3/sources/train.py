...
def train_model(model, num_classes, dataloaders, criterion, optimizer, device, dest_dir, num_epochs=25):
    ...
    for epoch in range(1, num_epochs+1):
        ...
        for phase in ['train', 'val']:
            ...
            if 0 == epoch%5:
                current_model_path = os.path.join(dest_dir, f"{epoch:04}_DeepLabV3_bestmodel_adam.pth")
                print(f"Save current model : {current_model_path}")
                torch.save(model.state_dict(), current_model_path)

        print()

    checkpoint_path = os.path.join(dest_dir, f"checkpoint_{epoch:04}_DeepLabV3_bestmodel_adam.pth")
    torch.save({
        'epoch': epoch,
